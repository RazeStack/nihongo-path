...
export function buildGrammarLesson(point: GrammarPoint, allPoints: GrammarPoint[], blockId: string, order: number): LessonDefinition {
  ...
  return {
    id: `n5-grammar-${point.id}`,
    blockId,
    title: point.title,
    description: point.formula,
    order,
    theory,
    miniCheck: buildMiniCheck(point, allPoints),
    itemIds: [point.id],
    xpReward: 50,
  }
}

function buildMiniCheck(point: GrammarPoint, allPoints: GrammarPoint[]): MiniCheckQuestion[] {
  ...
  return point.drillSentences.map((sentence, index): MiniCheckQuestion => {
    ...
    return {
      id: `n5-grammar-${point.id}-check-${index}`,
